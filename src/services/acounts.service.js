...
async function positions(options) {
  // console.log('options',options)
  if (!options && !options.id) throw new TypeError('id')

  let response = await connect.get({
    url: `${config.hostname}/accounts/${options.id}/positions/`,
    authorization: true,
  })
  return response
}

async function nonZeroPositions(options) {
  if (!options && !options.id) throw new TypeError('id')

  let response = await connect.get({
    url: `${config.endpoints.accounts}${options.id}/positions/`,
    authorization: true,
  })
  return response.results.filter((stock) => parseInt(stock.quantity) > 0)
}
